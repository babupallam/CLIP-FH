...
class PromptLearner(nn.Module):
    ...
    def __init__(self, classnames, cfg, clip_model,
        ...
        self.register_buffer("token_prefix", embed[:, :n_ctx + 1, :].clone().to(device))          # [SOS] + ctx slots
        self.register_buffer("token_suffix", embed[:, n_ctx + 1 + n_ctx:, :].clone().to(device))  # CLS + '.' + [EOS]
        ...
        text_out_dim = clip_model.visual.output_dim  # correct: 1024 for RN50, 512 for ViT
        print(f"[PromptLearner] ctx_dim: {ctx_dim}, output_dim: {text_out_dim}")
        self.proj = nn.Linear(ctx_dim, text_out_dim) if ctx_dim != text_out_dim else nn.Identity() # this condition makes it work only for RN50, not for vitb16
    ...
    def forward_batch(self, labels):
        ...
        return torch.stack(all_embeds).mean(dim=0)  # Average over templates
